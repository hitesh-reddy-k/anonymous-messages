document.addEventListener("DOMContentLoaded", () => {
    ...
    async function validateLink() {
        ...
        if (userId) {
            try {
                const response = await fetch(`https://anonymous-messages.vercel.app/anonymousMessages/username/${userId}`);
                if (!response.ok) {
                ...
                if (data.username) {
                    ...
                    sendButton.addEventListener("click", async () => {
                        ...
                        if (message) {
                            try {
                                const sendMessageResponse = await fetch(`https://anonymous-messages.vercel.app/anonymousMessages/send/${userId}`, {
                                    method: "POST",
                                    headers: {
    ...
    function extractUserIdFromLink(link) {
        const urlPattern = /http:\/\/localhost:5500\/anonymousMessages\/([a-fA-F0-9]{24})/;
